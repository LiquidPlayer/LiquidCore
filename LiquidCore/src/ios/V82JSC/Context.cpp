...
void GlobalContext::RemoveContextFromIsolate(IsolateImpl *iso, JSGlobalContextRef ctx)
{
    ...
    for (auto it=iso->m_exec_maps[ctx].begin(); it != iso->m_exec_maps[ctx].end(); ++it) {
                          ...
                          DeserializeInternalFieldsCallback internal_fields_deserializer)
{
    ...
    if (!i->m_nullContext.IsEmpty()) {
        ...
        */
        ...
        JSStringRef zBigIntPolyfill = JSStringCreateWithUTF8CString((const char*)jsbi_js);
        JSEvaluateScript(context->m_ctxRef, zBigIntPolyfill, global_o, 0, 0, &excp);
        assert(excp == 0);
        ...
        // Polyfill extras
        auto extras = exec(context->m_ctxRef, "return { isTraceCategoryEnabled : () => false, trace : () => true }", 0, nullptr);
        context->m_extras_binding_object.Reset(isolate, V82JSC::Value::New(context, extras).As<Object>());
    }
...
Local<Object> v8::Context::GetExtrasBindingObject()
{
    ...
    Local<Context> global_context = iso->m_global_contexts[gctx].Get(isolate);
    auto impl = ToImpl<V82JSC::GlobalContext>(global_context);
    return scope.Escape(impl->m_extras_binding_object.Get(isolate));
}
...
Local<PrimitiveArray> PrimitiveArray::New(Isolate* isolate, int length)
{
    ...
    int buffer = ReserveSize(length * sizeof(v8::internal::Object*) + sizeof(FixedArray));
    int size = (buffer - sizeof(FixedArray)) / sizeof(v8::internal::Object*);
    auto  newed = reinterpret_cast<FixedArray *>
        (HeapAllocator::Alloc(i, i->m_fixed_array_map,
                                 sizeof(FixedArray) + size*sizeof(v8::internal::Object *)));
    ...
    for (int j=0; j<length; j++) {
        newed->m_elements[j] = i->ii.heap()->root(R::kUndefinedValueRootIndex);
    }
    auto fa = CreateLocal<PrimitiveArray>(isolate, newed);
    return scope.Escape(fa);
}
...
Local<Primitive> PrimitiveArray::Get(Isolate* isolate, int index)
{
    EscapableHandleScope scope(isolate);
    auto impl = ToImpl<V82JSC::FixedArray>(this);
    auto item = impl->m_elements[index];

    return scope.Escape(CreateLocalSmi<Primitive>(reinterpret_cast<v8::internal::Isolate*>(isolate),
                                                  reinterpret_cast<v8::internal::Smi*>(item)));
}
