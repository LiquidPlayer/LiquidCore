...
void JSCPrivate::JSContextGroupAddHeapFinalizer(JSContextGroupRef group, JSHeapFinalizer finalizer, IsolateImpl *userData)
{
    ...
    userData->m_collection_thread = new std::thread([finalizer,userData](){
        volatile bool kill = userData->m_kill_collection_thread;
        while (!kill) {
            sleep(5);
            kill = userData->m_kill_collection_thread;
            if (!kill) {
    ...
    */
    //process_set_heap_finalizer(finalizer, userData);
}
...
void JSCPrivate::JSContextGroupRemoveHeapFinalizer(JSContextGroupRef group, JSHeapFinalizer finalizer, IsolateImpl *userData)
{
    ...
    */
    //process_stop_heap_finalizer(finalizer, userData);
}
