...
v8::String::Utf8Value::Utf8Value(Isolate *isolate, Local<v8::Value> obj)
{
    if (obj.IsEmpty()) {
        str_ = nullptr;
        length_ = 0;
        ...
        JSString str = JSValueToStringCopy(ToContextRef(context), value, &exception);
        if (exception) {
            str_ = nullptr;
        } else {
...
v8::String::Value::Value(Isolate* isolate, Local<v8::Value> obj)
{
    HandleScope scope(isolate);
    ...
    JSValueRef exception = nullptr;
    JSString s = JSValueToStringCopy(ToContextRef(context), value, &exception);
    if (exception) {
        s = "undefined";
    }
    length_ = (int) s.Length();
...
int v8::String::Length() const
{
    ...
    JSString s = JSValueToStringCopy(ToContextRef(context), value, 0);
    int r = (int) s.Length();
    return r;
}
 ...
 */
int v8::String::Utf8Length(Isolate* isolate) const
{
    HandleScope scope(isolate);
...
int v8::String::Write(Isolate* isolate,
                      uint16_t* buffer,
                      int start,
                      int length,
                      int options) const
...
// One byte characters.
int v8::String::WriteOneByte(Isolate* isolate,
                             uint8_t* buffer,
                             int start,
                             int length,
                             int options) const
...
int v8::String::WriteUtf8(Isolate *isolate,
                          char* buffer,
                          int length,
                          int* nchars_ref,
                          int options) const
 ...
 */
Local<v8::String> v8::String::Concat(Isolate* isolate, Local<String> left, Local<String> right)
{
    EscapableHandleScope scope(isolate);
    ...
    JSString sleft = JSValueToStringCopy(ToContextRef(context), left_, 0);
    JSString sright = JSValueToStringCopy(ToContextRef(context), right_, 0);

    size_t length_left = sleft.Length();
    size_t length_right = sright.Length();
