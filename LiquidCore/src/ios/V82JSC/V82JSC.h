...
struct JSString {
    JSString() : string_(0) {};
    JSString(JSStringRef str) : string_(str) {};
    JSString(const char* str) {
        string_ = JSStringCreateWithUTF8CString(str);
    }
    ~JSString() { if (string_) JSStringRelease(string_); }
    inline JSStringRef operator*() { return string_; }
    inline void operator=(JSStringRef str) { string_ = str; }
    inline void operator=(const char* str) { string_ = JSStringCreateWithUTF8CString(str); }
    inline bool operator!() { return string_ == 0; }
    inline JSStringRef* operator&() { return &string_; }
    inline JSStringRef Copy() { return JSStringRetain(string_); }
    inline int Length() { return string_ ? JSStringGetLength(string_) : 0; }
private:
    JSStringRef string_;
};
                              ...
                              const JSValueRef *argv, JSValueRef *pexcp=nullptr)
{
    ...
    if (!pexcp) {
        if (exception!=0) {
            JSString error = JSValueToStringCopy(ctx, exception, 0);
