...
Isolate * Isolate::New(Isolate::CreateParams const&params)
{
...

    {
        ...
        reinterpret_cast<internal::Isolate*>(isolate)->Init((v8::internal::StartupDeserializer *)&params);
    }
    
    impl->m_isolate_id = ArrayBufferInfo::NewIsolate();
    ...
    impl->m_global_contexts = std::map<JSGlobalContextRef, v8::Persistent<v8::Context>>();
    ...
    impl->m_external_strings = std::map<JSValueRef, v8::Persistent<v8::WeakExternalString>>();
    impl->m_internalized_strings = std::map<JSStringRef, v8::Persistent<v8::String>*>();
    ...
    impl->m_jsobjects = std::map<JSObjectRef, V82JSC::Value*>();
    ...
    impl->m_eternal_handles = std::vector<v8::Persistent<v8::Value> *>();
    ...
    // Create Map Objects
    impl->m_context_map = H::Map<H::Context>::New(impl, internal::BLOCK_CONTEXT_TYPE);
    impl->m_global_context_map = H::Map<H::GlobalContext>::New(impl, internal::BLOCK_CONTEXT_TYPE);
    ...
    impl->m_nullContextRef = (JSGlobalContextRef)ToContextRef(nullContext);
    JSGlobalContextRetain(impl->m_nullContextRef);
    ...
    impl->m_empty_string = JSValueMakeString(ToContextRef(nullContext), empty_string);
    JSValueProtect(ToContextRef(nullContext), impl->m_empty_string);
    JSStringRelease(empty_string);

    impl->m_negative_zero = exec(ToContextRef(nullContext), "return -0", 0, 0);
    JSValueProtect(ToContextRef(nullContext), impl->m_negative_zero);
...
bool IsolateImpl::PollForInterrupts(JSContextRef ctx, void* context)
{
    ...
    while (!empty) {
        ...
        interrupt.m_callback(ToIsolate(iso), interrupt.m_data);
        iso->m_pending_interrupt_mutex.lock();
...
void IsolateImpl::TriggerGCPrologue()
{
    ...
    for (auto i=m_gc_prologue_callbacks.begin(); i != m_gc_prologue_callbacks.end(); ++i) {
        if ((*i).m_callback)
            (*i).m_callback(ToIsolate(this), kGCTypeIncrementalMarking, kNoGCCallbackFlags);
        else
            (*i).m_callback_with_data(ToIsolate(this), kGCTypeIncrementalMarking, kNoGCCallbackFlags, (*i).m_data);
    }
}
...
void IsolateImpl::TriggerGCEpilogue()
{
    ...
    for (auto i=m_gc_epilogue_callbacks.begin(); i != m_gc_epilogue_callbacks.end(); ++i) {
        if ((*i).m_callback)
            (*i).m_callback(ToIsolate(this), kGCTypeScavenge, kNoGCCallbackFlags);
        else
            (*i).m_callback_with_data(ToIsolate(this), kGCTypeScavenge, kNoGCCallbackFlags, (*i).m_data);
    }
}
...
bool v8::internal::Isolate::Init(v8::internal::StartupDeserializer *des)
{
...
void Isolate::MemoryPressureNotification(MemoryPressureLevel level)
{
    static auto collectGarbage = [](IsolateImpl* iso) {
        ...
        for (auto i=iso->m_global_contexts.begin(); i!=iso->m_global_contexts.end(); ++i) {
            JSGarbageCollect(ToContextImpl(i->second.Get(ToIsolate(iso)))->m_ctxRef);
        }
        */
        ToIsolate(iso)->RequestGarbageCollectionForTesting(GarbageCollectionType::kFullGarbageCollection);
    };
    IsolateImpl* iso = ToIsolateImpl(this);
...
{
    printf("FIXME: Isolate::CheckMemoryPressure()");
}
...
void Isolate::Enter()
{
    ...
    ToIsolateImpl(this)->m_entered_count ++;
}
...
void IsolateImpl::ExitContext(Local<v8::Context> ctx)
{
    ...
    JSContextRef top = ToContextRef(thread->m_context_stack.top().Get(ToIsolate(this)));
    JSContextRef newctx = ToContextRef(ctx);
    assert(top == newctx);
...
void Isolate::Dispose()
{
    ...
    JSCPrivate::JSContextGroupRemoveHeapFinalizer(isolate->m_group, HeapFinalizerCallback, isolate);
    {
        ...
        while (!isolate->m_external_strings.empty()) {
            Local<WeakExternalString> wes = isolate->m_external_strings.begin()->second.Get(ToIsolate(isolate));
            auto impl = ToImpl<V82JSC::WeakExternalString>(wes);
    ...
    ArrayBufferInfo::DisposeIsolate(isolate->m_isolate_id);
    ...
    isolate->ii.heap()->TearDown();

    isolate->ii.global_handles()->TearDown();
    HeapAllocator::TearDown(isolate);
    
    JSGlobalContextRelease(isolate->m_nullContextRef);
    ...
    if (!iso->m_microtask_queue.empty()) {
        ...
        while (!iso->m_microtask_queue.empty()) {
            ...
            if (!microtask.m_callback.IsEmpty()) {
                ...
                task->Call(OperatingContext(this), Local<Value>(), 0, nullptr).FromMaybe(Local<Value>());
            } else {
 ...
 */
v8::MicrotasksPolicy Isolate::GetMicrotasksPolicy() const
{
...
{
    if (run_) ToIsolateImpl(isolate)->m_run_microtasks_depth ++;
}
...
{
    return ToIsolateImpl(isolate)->m_running_microtasks;
}
                                      ...
                                      Local<Value> data)
{
    IsolateImpl *iso = ToIsolateImpl(this);
    HandleScope scope(this);
    Local<Context> context = OperatingContext(this);
    ...
    if (data.IsEmpty()) {
        ...
        listener.data_ = ToJSValueRef(data, context);
...
void Isolate::VisitExternalResources(ExternalResourceVisitor* visitor)
{
    ...
    for(auto i=iso->m_external_strings.begin(); i!=iso->m_external_strings.end(); ++i) {
        ...
        auto wes = static_cast<V82JSC::WeakExternalString*>(FromHeapPointer(*(internal::Object**)*s));
        visitor->VisitExternalString(V82JSC::Value::New(ToContextImpl(iso->m_nullContext.Get(this)),
...
struct DisallowScopeImpl {
    ...
    {
        m_isolate = ToIsolateImpl(isolate);
...
{
    ToIsolateImpl(isolate)->m_suppress_microtasks = true;
}
...
{
    printf("Isolate::SetHostInitializeImportMetaObjectCallback");
}
...
{
    IsolateImpl *iso = ToIsolateImpl(this);
    iso->m_allow_wasm_code_generation_callback = callback;
}
