...
bool V82JSC::InstallExtension(Local<v8::Context> context, const char *extension_name,
...
#define is_space(c) (c == ' ' || c == '\t')
    ...
    Isolate *isolate = ToIsolate(iso);
    {
        ...
        for (auto i=native_function_names.begin(); i!=native_function_names.end(); ++i) {
            Local<v8::String> name = v8::String::NewFromUtf8(isolate, i->c_str(), v8::NewStringType::kNormal).ToLocalChecked();
            Local<v8::FunctionTemplate> native_function_template = extension.GetNativeFunctionTemplate(isolate, name);
            if (native_function_template.IsEmpty()) return false;
            Local<v8::Function> native_function = native_function_template->GetFunction(context).ToLocalChecked();
        ...
        Local<v8::String> source = v8::String::NewFromUtf8(isolate, output.c_str(), v8::NewStringType::kNormal).ToLocalChecked();
...
bool V82JSC::InstallAutoExtensions(v8::Local<v8::Context> context, std::map<std::string, bool>& loaded_extensions)
{
