    ...
    V8_ISOLATE_CTX(ctx->Context(), isolate, context)
        ...
        Local<FunctionTemplate> logt = FunctionTemplate::New(isolate,
            [](const FunctionCallbackInfo< Value > &info) {
                ...
                String::Utf8Value str(info.GetIsolate(), info[0]->ToString(info.GetIsolate()->GetCurrentContext()).ToLocalChecked());
...
JS_EXPORT void JSGlobalContextSetName(JSGlobalContextRef ctx, JSStringRef name)
{
    V8_ISOLATE(ctx->Context()->Group(), isolate)
        if (name) {
            ctx->Context()->Value()->SetEmbedderData(1, name->Value(isolate));
        } else {
            ctx->Context()->Value()->
                SetEmbedderData(1, Local<Value>::New(isolate,Undefined(isolate)));
        }
    V8_UNLOCK()
}
