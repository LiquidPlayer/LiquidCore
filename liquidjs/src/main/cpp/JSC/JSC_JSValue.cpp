    ...
    unsigned indent, JSValueRef* exceptionRef)
{
    ...
    VALUE_ISOLATE(CTX(ctxRef),valueRef,isolate,context,inValue)
        ...
        Local<Object> json = context->Global()->Get(String::NewFromUtf8(isolate, "JSON"))->ToObject(context).ToLocalChecked();
    ...
    JSValueRef* exceptionRef)
{
    ...
    VALUE_ISOLATE(CTX(ctxRef),valueRef,isolate,context,value)
        ...
        if (!string.IsEmpty()) {
            String::Utf8Value chars(isolate, string.ToLocalChecked());
    ...
    JSValueRef* exceptionRef)
{
    ...
    VALUE_ISOLATE(CTX(ctxRef),valueRef,isolate,context,value)
        ...
        if (!obj.IsEmpty()) {
            out = const_cast<JSObjectRef>(OpaqueJSValue::New(ctxRef, value->ToObject(context).ToLocalChecked()));
        } else {
            exception.Set(ctxRef, trycatch.Exception());
        }
    V8_UNLOCK()
